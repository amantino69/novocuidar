...
import { SchedulingSignalRService, SlotUpdateNotification, DayUpdateNotification, SlotProfessionalsUpdateNotification, ScheduleBlockNotification } from '@core/services/scheduling-signalr.service';
...
export class ReturnTabComponent implements OnDestroy, OnChanges {
  ...
  steps: { id: Step, label: string }[] = [
    ...
    { id: 'confirmation', label: 'Confirma��o' }
  ];
  ...
  private handleSlotUpdate(notification: SlotUpdateNotification): void {
    ...
    // Ignorar notifica��es durante cria��o de agendamento
    ...
    // Tamb�m verificar se � o slot selecionado no step de confirma��o (reserva em andamento)
    ...
    if ((isPendingReservation || isCurrentReservation || isSelectedSlotBeingReserved) && !notification.isAvailable) {
      console.log('[SignalR] Ignorando notifica��o - � nossa pr�pria reserva ou slot selecionado');
      return;
    }
  ...
  generateCalendar() {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      // Verificar se o dia está bloqueado para o profissional
      dayChecks.push(
